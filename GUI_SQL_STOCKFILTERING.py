...
class GUI:
    ...
    def create_interface(self):
        ...
        df = pd.read_sql_query("SELECT * FROM stocks LIMIT 1", self.conn)
        ...
        for i, (column, sanitized_col) in enumerate(zip(column_list, sanitized_columns)):
            ...
            self.cursor.execute(f"SELECT DISTINCT [{column}] FROM stocks WHERE [{column}] IS NOT NULL")
            ...
            self.cursor.execute(f"SELECT typeof([{column}]) FROM stocks LIMIT 1")
        ...
        self.tree = ttk.Treeview(self.inner_frame, columns=sanitized_columns, show="headings")
